use crate::commands::request::{resolve_env_vars, with_effective_auth};
...
pub async fn replay_history_entry(ctx: &AppContext, workspace_id: String, id: String) -> ApiResult<RequestRun> {
    ...
    // Re-resolve against the *live* active environment and folder/workspace
    // auth chain, the same way a replayed {{variable}} re-resolves rather
    // than reusing a frozen value from when the entry was first recorded —
    // see send_request's comment on the same tradeoff.
    let ws = ctx.storage.get_workspace(&workspace_id).await?;
    let vars = resolve_env_vars(ctx, &workspace_id, &ws.active_environment_id).await?;
    let effective = with_effective_auth(ctx, entry.request_snapshot.clone()).await?;
