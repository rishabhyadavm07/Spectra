...
pub async fn open_request(ctx: &AppContext, id: String) -> ApiResult<Request> {
    let res = ctx.storage.find_request(&id).await;
    println!("DEBUG: open_request({}) in DB {:?}, found: {}", id, ctx.storage.responses_dir, res.is_ok());
    res
}
...
async fn with_effective_auth(ctx: &AppContext, req: Request) -> ApiResult<Request> {
